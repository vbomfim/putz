...
**Last updated**: 2026-05-23 (T5 PR #159 CR-Pass-2 fixup — FR-004 frame set fully enumerated and grouped by ticket, with explicit forward-compat policy for unknown inbound frame types/fields; SEC-009 added pinning the trust model — all colleagues are full peers, no application-layer ACL planned for T5. Code-side fixes: claims now released eagerly when a duplicate-tab register evicts the previous colleague (was leaking until the TTL sweeper ran); claim TTL minimum bumped from 1s to 5s to match `SWEEP_INTERVAL`; `swarm_send` rewritten as an acknowledged RPC (`send_req`/`tool_response`) returning `{ delivered: true }` instead of a fire-and-forget `send_to`; Unicode bidi/zero-width "trojan" code points stripped from notify messages; Node `request()` capped at 100 in-flight RPCs (`PENDING_CAP`); SDK tool JSON schemas tightened with `pattern`/`minLength`/`maxLength`/`additionalProperties:false`; `onPostToolUse` dedupes per-tool-name within a 5s window so an LLM turn that fires `read_file` 30 times notifies peers once; `getContextBlock()` wraps output in `<swarm-context>...</swarm-context>` and escapes newlines/long peer messages. T5 PR — SC-005 production LOC budget revised from ≤900 to ≤1700 to absorb the swarm-coordination toolset: 6 SDK tools (`swarm_claim`/`swarm_release`/`swarm_check`/`swarm_list_claims`/`swarm_send`/`swarm_broadcast`), wire-frame additions (claim/release broadcasts + 5 RPC request frames + tool_response correlation), Node-side claim cache + inbox ring buffer + request/response correlation map, and the `onUserPromptSubmitted` SDK hook that prepends a swarm context block to every user turn. T4 PR #157 fixup — SC-004 production LOC budget revised from ≤1500 to ≤1700 to absorb the spawn-recipe loader + typed `LoadRecipeError` machinery + send-notify Tauri command + recipe-args bypass in `pty_spawn`. T3 PR #155 fixup — full-snapshot status semantics + `lastTenExitCodes` field; FR-012 latency contract clarified; SC-004 LOC budget revised to ≤2500 test LOC. T2 PR #156 fixup — SC-005 budget revised from ≤250 to ≤900 production LOC.)
...
- **FR-004**: The protocol MUST consist of these message types, partitioned by ticket:
  - **T1 — bootstrap & lifecycle**: `register`, `register_ack`, `heartbeat`, `disconnect`
  - **T1 — ambient messaging**: `notify`, `send_to`, `recv_from`
  - **T3 — full-snapshot status**: `roster_update`
  - **T4 — typed delivery**: `recv_notify`
  - **T5 — claim coordination**: `claim`, `release`, `claim_req`, `release_req`, `list_claims_req`, `broadcast_req`, `send_req`, `tool_response` (request/response RPC; `tool_response` correlates by `request_id`)

  Outbound encoding is strict (`deny_unknown_fields`); **inbound** decoding tolerates unknown frame `type` values and unknown extra fields on known types — receivers MUST silently ignore them. This forward-compat policy lets the coordinator ship a new frame type without immediately breaking older colleague extensions; adding a NEW required field to an EXISTING type is still a breaking change.
...
- **SC-004**: Total Rust LOC for the swarm subsystem after implementation: ≤ 1700 production LOC (excludes test code) and ≤ 2500 test LOC. Down from 3,019 in the prior HTTP-broker design. Zero LOC of HTTP server code. *Rationale: revised three times from the initial ≤ 600 estimate — first to ≤ 1200 after T1, then to ≤ 1500 after T3 PR #155 fixup, then to ≤ 1700 after T4 PR #157 fixup. The T4 increment covers the spawn-recipe loader (`spawn_recipe.rs` ~570 LOC), the typed `LoadRecipeError` enum, the `swarm_send_notify` Tauri command + `SwarmCoordinator::send_notify_to`, and the explicit-args bypass in `pty_spawn` so recipe-driven launches skip the login-shell allowlist. High-confidence concurrency tests (lifecycle race, debounce burst collapse, full-snapshot clear semantics, sentinel-cwd log redaction) and cross-platform path tests genuinely consume more lines than the initial estimate. Production-vs-test split documented separately so substantial regression test coverage (a project goal) does not push back against the size budget.*
- **SC-005**: Total Node LOC for the bundled Copilot CLI extension: ≤ 1700 production LOC (excludes test code). *Rationale: revised from ≤900 after T5 added the swarm-coordination toolset — 6 SDK tools, claim cache + inbox ring buffer, request/response correlation, and the `onUserPromptSubmitted` context-injection hook. Test code budget remains unbounded.*
...
- **SEC-009** *(trust model — colleagues are full peers)*: Every colleague that successfully `register`s on the swarm socket is a **full peer** of every other colleague. There is no read-only / observer / reduced-privilege colleague class. Any colleague can claim/release/list claims, send 1:1 messages to any other colleague, broadcast to the swarm, and read its own inbox. This mirrors the already-established trust boundary (SEC-002): the OS-permission layer (`chmod 600` socket / per-user pipe DACL) is the only authentication, and same-OS-user-as-Putz is the only authorization. We do **not** add an application-layer ACL because (1) it would defend against a threat model — same-UID-but-untrusted code — that the OS layer already disclaims, and (2) the concrete consumer is `gh copilot` running on the user's behalf in a Putz tab; the user already trusts that process with their shell. A future "shared by multiple OS users" or "remote colleague" mode would require a real authn/authz layer and is out of scope for T5.
...
| New: `extensions/copilot-swarm/` (Node) | **New** | Bundled Copilot CLI extension. ≤ 1700 production LOC (see SC-005 for rationale). |
