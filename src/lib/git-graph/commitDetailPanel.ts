...
export function renderCommitDetail(
  ...
  html.push(
    ...
    '<div class="detail-hash" title="' + escapeAttr(detail.hash) + '">' +
    escapeHtml(detail.abbreviatedHash) +
    '</div>' +
    '<div class="detail-subject">' + escapeHtml(detail.subject) + '</div>' +
    '</div>',
  );
  ...
  html.push(
    ...
    ' <span class="detail-email">&lt;' + escapeHtml(detail.authorEmail) + '&gt;</span>' +
    '</div>' +
  ...
  if (detail.parentHashes.length > 0) {
    const parentLinks = detail.parentHashes.map((h) =>
      '<span class="detail-parent-hash" data-hash="' + escapeAttr(h) + '">' +
      escapeHtml(h.slice(0, 7)) + '</span>',
    ).join(', ');
    html.push(
      '<div class="detail-section detail-parents">' +
      '<span class="detail-label">Parent' + (detail.parentHashes.length > 1 ? 's' : '') + ':</span> ' +
      parentLinks +
      '</div>',
    );
  }
  ...
  if (detail.body && detail.body.trim()) {
    html.push(
      ...
      '<pre class="detail-body-text">' + escapeHtml(detail.body.trim()) + '</pre>' +
      '</div>',
    );
  }
  ...
  html.push(
    ...
    '<span class="detail-files-count">' + files.length + '</span>' +
    '</div>',
  );
  ...
  if (files.length === 0) {
    ...
    for (const file of files) {
      html.push(
        '<li class="detail-file-item" data-path="' + escapeAttr(file.path) + '" ' +
        'data-hash="' + escapeAttr(detail.hash) + '">' +
        '<span class="status ' + sanitizeStatusClass(file.status) + '">' +
        escapeHtml(file.status) + '</span> ' +
        '<span class="detail-file-path">' + escapeHtml(file.path) + '</span>' +
        '</li>',
      );
    }
    html.push('</ul>');
