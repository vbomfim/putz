...
export function MarkdownToolbar({ editorRef }: MarkdownToolbarProps) {
  const wrapInline = useCallback(
    (left: string, right: string = left, placeholder = "text") => {
  ...
  const insertBlock = useCallback(
    (block: string) => {
      ...
      editor.executeEdits("md-block", [
        { range: sel, text: block, forceMoveMarkers: true },
      ]);
      editor.focus();
    },
    [editorRef],
