...
export const terraformLanguageConfig: monaco.languages.LanguageConfiguration = {
  ...
  indentationRules: {
    increaseIndentPattern:
      /^\s*(resource|data|variable|output|locals|module|provider|terraform|dynamic|content|provisioner|connection|backend|lifecycle|for_each|count)\b.*\{\s*$/,
...
export const terraformTokensProvider: monaco.languages.IMonarchLanguage = {
  ...
  tokenizer: {
    root: [
      ...
      [
        /\b(resource|data|variable|output|locals|module|provider|terraform|backend|provisioner|connection|dynamic|content|moved|import|check)\b/,
        "keyword.block",
      ],

      // Meta-arguments
      [
        /\b(for_each|count|depends_on|lifecycle|create_before_destroy|prevent_destroy|ignore_changes|replace_triggered_by|providers|source|version)\b/,
        "keyword.meta",
      ],
      ...
      // Type keywords
      [
        /\b(string|number|bool|list|map|set|object|tuple|any|optional)\b/,
        "type",
      ],
      ...
      [
        /\b(aws|azurerm|google|azuread|helm|kubernetes|vault|consul|nomad|tfe|tls|random|null|local|external|archive|http|dns|time)_\w+\b/,
        "type.resource",
      ],

      // Attribute references (var., local., module., data., each., self., count.)
      [
        /\b(var|local|module|data|each|self|count|path|terraform)\b(?=\.)/,
        "variable.predefined",
      ],
