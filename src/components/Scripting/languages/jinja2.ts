...
export const jinja2TokensProvider: monaco.languages.IMonarchLanguage = {
  ...
  keywords: [
    ...
    "raw", "endraw", "autoescape", "endautoescape",
    "with", "endwith", "trans", "endtrans", "pluralize",
    "do", "continue", "break",
    "as", "in", "not", "and", "or", "is",
    "recursive", "scoped", "ignore missing",
    "true", "false", "none", "True", "False", "None",
    "loop", "caller", "varargs", "kwargs",
  ],

  // Built-in filters
  filters: [
    "abs", "attr", "batch", "capitalize", "center", "count",
    "d", "default", "dictsort", "e", "escape", "filesizeformat",
    "first", "float", "forceescape", "format", "groupby",
    "indent", "int", "items", "join", "last", "length",
    "list", "lower", "map", "max", "min", "pprint",
    "random", "reject", "rejectattr", "replace", "reverse",
    "round", "safe", "select", "selectattr", "slice", "sort",
    "string", "striptags", "sum", "title", "tojson", "trim",
    ...
    "from_yaml", "from_json", "bool", "ternary",
    "regex_search", "regex_replace", "regex_findall",
    "combine", "dict2items", "items2dict",
    "subelements", "zip", "zip_longest",
    "ipaddr", "ipv4", "ipv6", "ipsubnet", "nthhost",
    "hwaddr", "macaddr",
    "hash", "password_hash", "b64encode", "b64decode",
    "flatten", "product", "permutations", "combinations",
    "type_debug", "mandatory", "comment",
  ],

  // Built-in tests
  tests: [
    "callable", "defined", "divisibleby", "eq", "equalto",
    "escaped", "even", "false", "ge", "gt", "greaterthan",
    "in", "iterable", "le", "lower", "lt", "lessthan",
    "mapping", "ne", "none", "number", "odd", "sameas",
    "sequence", "string", "true", "undefined", "upper",
    // Ansible-specific tests
    "match", "search", "regex", "version",
    "subset", "superset", "all", "any",
    "changed", "failed", "succeeded", "skipped",
    "file", "directory", "link", "exists", "abs",
  ],
  ...
  tokenizer: {
    ...
    jinjaCommon: [
      ...
      // Keywords
      [/\b(for|endfor|if|elif|else|endif|block|endblock|extends|include|import|from|macro|endmacro|call|endcall|filter|endfilter|set|endset|raw|endraw|with|endwith|do|continue|break|as|in|not|and|or|is|recursive|scoped)\b/, "keyword.jinja"],

      // Boolean / None
      [/\b(true|false|none|True|False|None)\b/, "keyword.constant.jinja"],

      // Loop variable
      [/\b(loop)\.(index|index0|revindex|revindex0|first|last|length|cycle|depth|depth0|previtem|nextitem|changed)\b/, "variable.predefined.jinja"],
    ...
    provideCompletionItems(model, position) {
      ...
      if (textBefore.match(/\|\s*\w*$/)) {
        const filters: [string, string][] = [
          ...
          ["regex_replace", "regex_replace('${1:pattern}', '${2:replacement}')"],
          ["ipaddr", "ipaddr"],
          ["ipv4", "ipv4"],
        ...
        for (const [label, insert] of filters) {
          suggestions.push({
            ...
            insertText: insert,
            insertTextRules: insert.includes("$") ? CompletionItemInsertTextRule.InsertAsSnippet : undefined,
            range,
          });
        }
        return { suggestions };
      ...
      const snippets: [string, string, string][] = [
        ...
        ["for loop", "{% for ${1:item} in ${2:items} %}\n${3}\n{% endfor %}", "For loop block"],
        ["if block", "{% if ${1:condition} %}\n${2}\n{% endif %}", "If block"],
        ["if/else", "{% if ${1:condition} %}\n${2}\n{% else %}\n${3}\n{% endif %}", "If/else block"],
        ["macro", "{% macro ${1:name}(${2:args}) %}\n${3}\n{% endmacro %}", "Macro definition"],
        ["block", "{% block ${1:name} %}\n${2}\n{% endblock %}", "Template block"],
        ["interface template", "interface {{ ${1:interface_name} }}\n description {{ ${2:description} }}\n ip address {{ ${3:ip} }} {{ ${4:mask} }}\n no shutdown", "Cisco interface template"],
        ["loop with index", "{% for ${1:item} in ${2:items} %}\n${3:{{ loop.index }}. {{ ${1} }}}\n{% endfor %}", "Loop with index"],
      ];
    ...
    const validate = () => {
      ...
      if (openExpr > 0) {
        markers.push({
          ...
          startLineNumber: lines.length, startColumn: 1,
