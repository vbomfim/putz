...
export function VaultTab() {
  ...
  const [form, setForm] = useState({ name: "", username: "", secret: "", type: "password" as "password" | "key_passphrase" });
  const [keyForm, setKeyForm] = useState({ name: "", algorithm: "ed25519" as "ed25519" | "rsa-4096", passphrase: "" });
  ...
  const handleCopyPassword = useCallback(async (id: string) => {
    try {
      const cred = await invoke<{ meta: CredentialMeta; secret: string }>("vault_get", { id });
  ...
  const handleEditCred = useCallback(async (id: string) => {
    try {
      const cred = await invoke<{ meta: CredentialMeta; secret: string }>("vault_get", { id });
      setForm({
        name: cred.meta.name,
  ...
  const handleSaveCred = useCallback(async () => {
    if (!form.name.trim() || !form.username.trim() || !form.secret.trim()) return;
    setIsSaving(true);
    try {
  ...
  // ── SSH Key actions ─────────────────────────────────────────
  const handleCopyPubKey = useCallback(async (id: string) => {
    try {
  ...
  const filteredCreds = creds.filter((c) =>
    c.name.toLowerCase().includes(lowerFilter) || c.username.toLowerCase().includes(lowerFilter),
  );
  const filteredKeys = keys.filter((k) =>
    k.name.toLowerCase().includes(lowerFilter) || k.fingerprint.toLowerCase().includes(lowerFilter),
  );
  ...
  if (loading) {
    return <div className="vault-tab"><div className="vault-tab__loading">Loading vault…</div></div>;
  }
    ...
    <div className="vault-tab" onKeyDown={handleFormKeyDown}>
      ...
      <div className="vault-tab__header">
        ...
        <button
          ...
          onClick={() => {
            ...
            else { setIsAdding(true); setSection("keys"); }
          }}
        >
          +
        </button>
      ...
      {section === "creds" && (
        <div className="vault-tab__list">
            ...
            <div className="vault-tab__inline-form">
              ...
              <div className="vault-tab__form-row">
                ...
                <select
                  ...
                  value={form.type}
                  onChange={(e) => setForm({ ...form, type: e.target.value as "password" | "key_passphrase" })}
                >
                  <option value="password">Password</option>
                  <option value="key_passphrase">Key Passphrase</option>
                </select>
                <button className="vault-tab__form-save" onClick={handleSaveCred} disabled={isSaving}>
                  {isSaving ? "…" : "✓"}
                </button>
                <button className="vault-tab__form-cancel" onClick={handleCancelEdit}>✕</button>
              </div>
            </div>
          )}
          ...
          {/* Credential list */}
          {filteredCreds.map((c) => (
            <div key={c.id} className={`vault-tab__item ${editingId === c.id ? "vault-tab__item--editing" : ""}`}>
              ...
              <div className="vault-tab__item-actions">
                <button className="vault-tab__action" onClick={() => handleCopyUsername(c.username)} title="Copy username">👤</button>
                <button className="vault-tab__action" onClick={() => handleCopyPassword(c.id)} title="Copy password">📋</button>
                <button className="vault-tab__action" onClick={() => handleEditCred(c.id)} title="Edit">✏️</button>
                <button className="vault-tab__action vault-tab__action--danger" onClick={() => handleDeleteCred(c.id)} title="Delete">🗑</button>
              </div>
            </div>
          ))}
          {filteredCreds.length === 0 && !isAdding && (
            <div className="vault-tab__empty">
              {filter ? "No matching credentials" : "No credentials — click + to add"}
            </div>
          )}
        </div>
      )}
            ...
            <div className="vault-tab__inline-form">
              <div className="vault-tab__form-row">
                ...
                <select
                  ...
                  onChange={(e) => setKeyForm({ ...keyForm, algorithm: e.target.value as "ed25519" | "rsa-4096" })}
                >
                ...
                />
                <button className="vault-tab__form-save" onClick={handleGenerateKey} disabled={isGenerating}>
                  {isGenerating ? "…" : "⚡"}
                </button>
                <button className="vault-tab__form-cancel" onClick={handleCancelEdit}>✕</button>
              </div>
          ...
          {filteredKeys.map((k) => (
            <div key={k.id} className="vault-tab__item">
              <div className="vault-tab__item-info">
                ...
                <span className="vault-tab__item-badge">{k.algorithm === "ed25519" ? "ED25519" : "RSA"}</span>
                <span className="vault-tab__item-fingerprint" title={k.fingerprint}>
                  {k.fingerprint.substring(0, 24)}…
                </span>
              </div>
              <div className="vault-tab__item-actions">
                <button className="vault-tab__action" onClick={() => handleCopyPubKey(k.id)} title="Copy public key">📋</button>
                <button className="vault-tab__action vault-tab__action--danger" onClick={() => handleDeleteKey(k.id)} title="Delete">🗑</button>
              </div>
            </div>
          ))}
          {filteredKeys.length === 0 && !isAdding && (
            <div className="vault-tab__empty">
              {filter ? "No matching keys" : "No SSH keys — click + to generate"}
            </div>
          )}
        </div>
      )}
