...
import {
  renderGraph,
  highlightCommit,
} from "../../lib/git-graph/graphRenderer";
...
export function GitGraph({ repoPath }: GitGraphProps) {
  ...
  const [worktrees, setWorktrees] = useState<
    { path: string; branch: string; hash: string; bare: boolean }[]
  >([]);
  ...
  const loadGraph = useCallback(
    async (filePath?: string, repoOverride?: string) => {
      ...
      try {
        const logArgs: {
          ...
          maxCount: number;
          filePath?: string;
        } = { repoPath: repo, maxCount: 500 };
        if (filePath) logArgs.filePath = filePath;

        const [logRaw, statusRaw, remotesRaw, headHash, wtListRaw] =
          await Promise.all([
  ...
  const openWorkingTreeDiff = useCallback(
    async (file: GitFileChange, staged: boolean) => {
      try {
        ...
        const workingContent = await invoke<string>("file_read", {
          path: fullPath,
        });
          ...
          if (tab) {
            useLayoutStore
              ...
              .renameTab(
                region.id,
                tab.id,
                `${fileName} (${staged ? "staged" : "modified"})`,
              );
          }
        }
      } catch (e) {
  ...
  useEffect(() => {
    ...
    const handleCommitClick = async (hash: string) => {
      ...
      try {
        const raw = await invoke<string>("git_show", {
          repoPath: activeRepo.current,
          hash,
        });
            ...
            onOpenFileDiff: async (h, fp) => {
              try {
                ...
                const [oldContent, newContent] = await Promise.all([
                  parentHash
                    ? invoke<string>("git_file_at_commit", {
                        repoPath: activeRepo.current,
                        hash: parentHash,
                        filePath: fp,
                      })
  ...
  if (loading) {
    return (
      <div className="git-graph git-graph--loading">Loading git history…</div>
    );
  }
  ...
  return (
    <div className="git-graph">
      <div className="git-graph__toolbar">
        <button
          onClick={() => loadGraph(fileFilter.trim() || undefined)}
          title="Refresh"
        >
          ↻
        </button>
        ...
        {fileFilter && (
          <>
            <button onClick={applyFilter} title="Apply filter">
              🔍
            </button>
            <button
              onClick={clearFilter}
              title="Clear filter"
              className="git-graph__filter-clear"
            >
              ✕
            </button>
          </>
      ...
      {wtStatus &&
        (wtStatus.staged.length > 0 ||
          wtStatus.unstaged.length > 0 ||
          wtStatus.untracked.length > 0) && (
          <GitStatus status={wtStatus} onFileClick={openWorkingTreeDiff} />
        )}
      {worktrees.length > 1 && (
        <WorktreeList
          worktrees={worktrees}
          currentPath={activeRepo.current}
          onOpen={(path) => {
...
function GitStatus({
  ...
  return (
    <div className="git-graph__status">
      <button
        className="git-graph__status-header"
      ...
      >
        ...
        <span className="git-graph__status-counts">
          {status.staged.length > 0 && (
            <span style={{ color: STATUS_COLORS.added }}>
          ...
          {total === 0 && (
            <span style={{ color: STATUS_COLORS.untracked }}>clean</span>
          )}
        </span>
      </button>
      {expanded && (
        <div className="git-graph__status-files">
          {status.staged.map((f) => (
            <button
              ...
              onClick={() => onFileClick(f, true)}
            >
              <span
                className="git-graph__status-icon"
          ...
          {status.untracked.map((p) => (
            <button
              key={"t-" + p}
              className="git-graph__status-file"
              onClick={() => onFileClick({ path: p, status: "added" }, false)}
            >
              <span
                className="git-graph__status-icon"
                style={{ color: STATUS_COLORS.untracked }}
              >
                ?
              </span>
...
function parseWorktreeList(
  ...
  let current: { path: string; branch: string; hash: string; bare: boolean } = {
    path: "",
    branch: "",
    hash: "",
    bare: false,
          ...
          {nonBare.map((wt) => {
            ...
            return (
              <button
                ...
                style={
                  isCurrent ? { opacity: 0.6, cursor: "default" } : undefined
                }
                title={wt.path}
              >
                <span
                  className="git-graph__status-icon"
                  style={{ color: isCurrent ? "#50fa7b" : "#89b4fa" }}
                >
                  {isCurrent ? "★" : "⎇"}
                </span>
                <span className="git-graph__status-path">
                  {wt.branch || wt.hash.slice(0, 7)}
                </span>
                <span
                  className="git-graph__status-label"
                  style={{ color: "#6c7086" }}
                >
                  {wt.path.split("/").pop()}
                </span>
