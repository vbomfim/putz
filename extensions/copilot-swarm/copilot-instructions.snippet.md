...
1. **Check** — call `swarm_check({ resource })` first. If `held: true`
   and the holder is not you, **stop** and ask the user how to proceed
   ("colleague X holds `git-worktree` for ~3 min: `pulling main`. Wait
   or override?"). Do **not** silently override another colleague's
   claim.
2. **Claim** — `swarm_claim({ resource, ttl_secs, message })` with a
   realistic TTL (90s for a quick git pull, 600s for a deploy) and a
   short human-readable message. If `granted: false`, treat as step 1.
3. **Do the work**.
...
The extension automatically prepends a `<swarm-context>` block to every
user prompt listing active peers, current claims, and unread inbox
messages from peers. **Read it before acting.** If a peer holds a
resource you were about to touch, stop and consult the user.
